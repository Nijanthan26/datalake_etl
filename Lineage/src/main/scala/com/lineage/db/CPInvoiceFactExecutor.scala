...
inner join antuit_stage.hj_t_bmm_charge chg on inv.invoice_id = chg.invoice_idleft join antuit_stage.hj_t_bmm_contract_invoice_type cit on inv.contract_invoice_type_id = cit.contract_invoice_type_idinner join antuit_stage.hj_t_bmm_contract_master cm on cit.contract_id = cm.contract_idleft join antuit_stage.hj_t_bmm_customer cust on cm.customer_id = cust.customer_id
) inv_amt
group by 
inv_amt.customer_code
,inv_amt.generated_date
...
rate_history_factMRS.write.mode("append").jdbc(url, "invoice_fact_imp", prop)
rate_history_factHJ.write.mode("append").jdbc(url, "invoice_fact_imp", prop)
